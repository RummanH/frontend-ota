...
export class FlightListComponent {
  ...
  private location2 = inject(Location);
  ...
  ngOnInit() {
    ...
    const encryptedSearchData = (this.location2.getState() as any)['encryptedSearchData'];

    const searchData = JSON.parse(this.encrDecr.getString(environment.encryptionKey, encryptedSearchData));

    this.searchForm = this.fb.group({
      journeyType: [searchData.journeyType],
      segments: this.fb.array(this.buildSegmentArray(searchData.segments)),
      classType: [searchData.classType, Validators.required],
      adults: [searchData.adults, [Validators.required, Validators.min(1)]],
      children: [searchData.children, [Validators.min(0)]],
      kids: [searchData.kids, [Validators.min(0)]],
      infants: [searchData.infants, [Validators.min(0)]],
      carriersCode: [searchData.carriersCode || ''],
    });

    this.activatedRoute.queryParamMap.subscribe((params) => {
      this.journeyType = this.searchForm.value?.journeyType || 1;
      this.totalTravelars = this.adults + this.children + this.infants + this.kids;

      // public int ApiUserID { get; set; }

      //      public Flex Flex { get; set; }
      //      public bool? IsOwnCountry { get; set; }
      //      public bool? IsSpecialTexRedumtion { get; set; }
      //      public List<int> ChildrenAges { get; set; }
      //      public SortingType SortingType { get; set; }
      //      public bool IsAppsUserSearch { get; set; }
      //      public string AndroidVersion { get; set; }
      //      public Dictionary<string, decimal> ApiCurrency { get; set; }
      ...
      if (this.searchForm.value?.journeyType === 1 || this.searchForm.value?.journeyType === 2) {
        const childrenAges: number[] = [];

        for (let i = 0; i < this.searchForm.value?.kids; i++) {
          childrenAges.push(3);
        }

        for (let i = 0; i < this.searchForm.value?.children; i++) {
          childrenAges.push(6);
        }
        requestModel = {
          JourneyType: this.searchForm.value?.journeyType,
          Origin: this.searchForm.value?.segments[0]?.origin?.code,
          Destination: this.searchForm.value?.segments[0]?.destination?.code,
          DepartureDate: this.apiDateToString(this.searchForm.value?.segments[0]?.departureDate),
          ReturnDate: this.searchForm.value?.segments[0]?.returnDate ? this.apiDateToString(this.searchForm.value?.segments[0]?.returnDate) : '',
          ClassType: this.searchForm.value?.classType?.name,
          NoOfInfant: this.searchForm.value?.kids + this.searchForm.value?.infants,
          NoOfChildren: this.searchForm.value?.children,
          NoOfAdult: this.searchForm.value?.adults,
          CarriersCode: this.searchForm.value?.carriersCode.split('/').filter(Boolean),
          IsFlexSearch: this.searchForm.value?.carriersCode?.length > 0,
          ChildrenAges: childrenAges,
        };
      }

      if (this.searchForm.value?.journeyType == 3) {
        const childrenAges: number[] = [];
        ...
        const Multicities = this.searchForm?.value?.segments?.map((s) => {
          return {
            Origin: s.origin?.code,
            Destination: s.destination?.code,
            DepartureDate: this.apiDateToString(s.departureDate),
          };
        });
        requestModel = {
          JourneyType: this.searchForm.value?.journeyType,
      ...
      this.authService?.searchFlights(requestModel, this.searchForm.value?.journeyType).subscribe(
        (res) => {
          if (res.Success) {
            ...
            this.originalFlightList = res.Payload;
  ...
  filterAfterFetch(currentFlights: any) {
    ...
    this.filter_params.aircraft = currentFlights.map((data) => data.Onwards.flat()[0].Equipment).filter((x: any, i: any, a: any) => x && a.indexOf(x) === i);

    this.filter_params.baggage = currentFlights.map((data) => data.Onwards.flat()[0].AirBaggageAllowance).filter((x: any, i: any, a: any) => x && a.indexOf(x) === i);

    if (this.journeyType != 3) {
      for (let i = 0; i < currentFlights.length; i++) {
        const singleListData: any = currentFlights[i];
        const length_onward = singleListData.Onwards.length;
        const length_return = singleListData.Returns.length;
        if (length_onward > 1) {
          for (let j = 0; j < length_onward; j++) {
            if (j !== length_onward - 1) {
              if (!this.filter_params.onward_layover_airport.includes(singleListData.Onwards[j].DestinationAirPortName)) {
                this.filter_params.onward_layover_airport.push(singleListData.Onwards[j].DestinationAirPortName);
              }
              if (!this.filter_params.onward_destination_airport.includes(singleListData.Onwards[length_onward - 1].DestinationAirPortName)) {
                this.filter_params.onward_destination_airport.push(singleListData.Onwards[length_onward - 1].DestinationAirPortName);
              }
            }
          }
        }
        if (length_return > 1) {
          for (let j = 0; j < length_return; j++) {
            if (j !== length_return - 1) {
              if (!this.filter_params.return_layover_airport.includes(singleListData.Returns[j].DestinationAirPortName)) {
                this.filter_params.return_layover_airport.push(singleListData.Returns[j].DestinationAirPortName);
              }
              if (!this.filter_params.return_destination_airport.includes(singleListData.Returns[length_return - 1].DestinationAirPortName)) {
                this.filter_params.return_destination_airport.push(singleListData.Returns[length_return - 1].DestinationAirPortName);
              }
            }
          }
        }
  ...
  compareDepartureTime = (flightA: any, flightB: any) => {
    const a = this.journeyType == 3 ? flightA.Onwards[0][0].DepartureTime : flightA.Onwards[0].DepartureTime;
    const b = this.journeyType == 3 ? flightB.Onwards[0][0].DepartureTime : flightB.Onwards[0].DepartureTime;
    ...
    if (this.modify_filter_params.aircraft.length) {
      tempList = tempList.filter((currentFlight) => {
        return this.modify_filter_params.aircraft.some((aircraft: string) => {
          return aircraft === currentFlight.Onwards.flat()[0].Equipment;
        });
      });
    }
    ...
    if (this.modify_filter_params.onward_layover_airport.length) {
      tempList = tempList.filter((currentFlight): any => {
        return this.modify_filter_params.onward_layover_airport.some((onwardLayoverAirport): any => {
          return currentFlight.Onwards.flat().some((onward, i: number): any => {
    ...
    if (this.modify_filter_params.return_layover_airport.length) {
      tempList = tempList.filter((currentFlight) => {
        return this.modify_filter_params.return_layover_airport.some((returnLayoverAirport) => {
          return currentFlight.Returns.flat().some((returns, i: number): any => {
